...
class ConsentProvision:
    ...
    actor: Optional[List[ConsentProvisionActor]] = field(default_factory=list)  # Who or what is controlled by this provision. Use group to identify a set of actors by some proper...
    ...
    data: Optional[List[ConsentProvisionData]] = field(default_factory=list)  # The resources controlled by this provision if specific resources are referenced.
    expression: Optional[Expression] = None  # A computable (FHIRPath or other) definition of what is controlled by this consent.
    provision: Optional[List['ConsentProvision']] = field(default_factory=list)  # Provisions which provide exceptions to the base provision or subprovisions.
...
class Consent(FHIRResource):
    ...
    provision: Optional[List[ConsentProvision]] = field(default_factory=list)  # An exception to the base policy of this consent. An exception can be an addition or removal of ac...