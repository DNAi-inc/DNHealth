...
"""
Base classes for FHIR R5 resources.

FHIR R5 resource hierarchy (same as R4):
...
- CanonicalResource: Extends DomainResource (adds canonical fields)
- MetadataResource: Extends CanonicalResource (adds metadata fields)

R5 maintains the same base structure as R4 for compatibility, but may have
additional fields or changed constraints in specific resources.
"""

from dataclasses import dataclass, field
from typing import List, Optional, Dict, Any, TYPE_CHECKING

# Import R4 types for compatibility - R5 uses same base types
...
class Meta:
    """
    FHIR R5 Meta element.
    
    Metadata about the resource.
    R5 Meta structure is compatible with R4.
    """
...
class Resource:
    """
    FHIR R5 Resource base class.
    
    Base abstract class for all FHIR R5 resources.
    All resources have: resourceType, id, meta, implicitRules, language
    Structure is compatible with R4 for interoperability.
    """
...
class DomainResource(Resource):
    """
    FHIR R5 DomainResource base class.
    ...
    Structure is compatible with R4.
    """
...
class CanonicalResource(DomainResource):
    """
    FHIR R5 CanonicalResource base class.
...
class MetadataResource(CanonicalResource):
    """
    FHIR R5 MetadataResource base class.
...
# Alias for backward compatibility and consistency with R4
